...
begin
  ...
  if pfDevice=FBitmap.PixelFormat then FBitmap.PixelFormat:=pf24bit;
  ...
  then begin
   ...
   if FBitmap.Height=1
   then FDelta:=0
   else FDelta := Integer(FBitmap.Scanline[1]) - FStart;
  End;
...
begin
 if (x<0) or (x>=FBitmap.Width) or (y<0) or (y>=FBitmap.Height) then Exit;
 SetPixel(X, Y, Value);
end;
