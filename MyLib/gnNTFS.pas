     ...
     while NextEntry do
      begin
       ...
       if (sM=false)and(AnsiCompareText(sT,'::$DATA')=0) then
        begin
         sM:=true;
         sN:=sN+'Основной поток: '+sT+'; Размер: '+IntToStr(tSI^.Size)+' байт'+CRLF;
        end else
        begin
         sN:=sN+'Опциональный поток: '+sT+'; Размер: '+IntToStr(tSI^.Size)+' байт'+CRLF;
         if Delete then
          if DeleteFile(FileName+sT) then sN:=sN+'Удален!'+CRLF;
        end;
