...
/// кто ссылается на них. Так должно быть.
...
////////////////////// x //////////////////////
 ...
 /// prop TimeTableX -
 /// prop Title
 /// prop Visible - 16.06.2012: возможно это свойство должно быть в TLesson
 ///
 ...
 /// prop Parent - Индекс предмета от которого произошёл текущий
  ...
  protected
    ...
    function GetKlass: TKlass; virtual;
    function GetNameIndex: Integer; virtual;
    // Бал по СанПин нужно сделать независимым
    function GetSanPIN: Integer; virtual;
  ...
  public
    ...
    property Klass: TKlass read GetKlass write SetKlass;
    ...
    property NameIndex: Integer read GetNameIndex write SetNameIndex;
 ...
 // TLesson - ***
  ...
  protected
    function GetKlass: TKlass; override;
    function GetLongName: string; override;
    function GetShortName: string; override;
    function GetNameIndex: Integer; override;
    function GetSanPIN: Integer; override;
  public
 ...
 ///   prop Subjects: TSubjects (RW); - Информация о предметах
  ...
  public
    constructor Create;
    destructor Destroy; override;
    procedure Add(LessonIndex: Integer; Subject: TSubject); overload;
    procedure Add(LessonIndex: Integer; Lesson: TLesson); overload;
    ...
    procedure DeleteFromState(LessonIndex: Integer; Lesson: TLesson);
    procedure AddToState(LessonIndex: Integer; var Lesson: TLesson);
    property AsJsonObject: ISuperObject read GetAsJsonObject write SetAsJsonObject;
 ...
 // TSubjects - информация о всех предметах
  ...
  private
    ...
    function GetItemAsJsonObject(Index: Integer): ISuperObject;
    procedure SetItemAsJsonObject(Index: Integer; const Value: ISuperObject);
  public
    constructor Create;
    destructor Destroy; override;
    ...
    property ItemAsJsonObject[Index:Integer]: ISuperObject read GetItemAsJsonObject write SetItemAsJsonObject;
    property Kabinets: TKabinets read FKabinets;
    property Klasses: TKlasses read FKlasses;
...
begin
 // Получить информацию о предмете в виде одной строки
  ...
  if MaxInd > -1 then begin
    ...
    st := st + ObrTime(LessonAtWeek) + '  ||   ' + tuSt[Complexion] + '  ||   ';
    s := '';
    for I := 0 to TeacherCount - 1 do begin
      if s > '' then
        s := s + ', ';
      if Teachers[I] <> nil then
        s := s + Teachers[I].Name;
    end;
    st := st + s;
    s := '';
...
function TSubject.GetKlass: TKlass;
begin
  Result := FKlass; 
end;
...
function TSubject.GetNameIndex: Integer;
begin
  Result := FNameIndex; 
end;
...
begin
 // Задать список кабинетов в которых проходит урок
  if (x < 0) or (x > MaxInd) or (x >= Length(FKabinets)) then
    Exit;
  if FTeachers[x] <> nil then begin
    if self is TLesson then
      Coll := FKabinets[x].Lessons
    else
      Coll := FKabinets[x].Subjects;
    Coll.Delete(self);
  end;
  FKabinets[x] := Value;
  if ItemIndex = -1 then
...
begin
 // Задать список учителей, которые ведут этот предмет
  ...
  if FTeachers[x] <> nil then begin
    if self is TLesson then
      Coll := FTeachers[x].Lessons
    else
      Coll := FTeachers[x].Subjects;
    Coll.Delete(self);
  end;
  FTeachers[x] := Value;
  if ItemIndex = -1 then
...
//////////////////////////////////////////////////
{private}
function TTimeTable.GetAsJsonObject: ISuperObject;
var
  I: Integer;
  J: Integer;
  supLessons: TSuperArray;
  supLesson: ISuperObject;
  CountOfLessonsInTT: Integer;
  ParentIndex: Integer;
begin
  Result := SO;
  Result.O['Subjects'] := Subjects.AsJsonObject; 
  Result.O['Items'] := SO('[]');
  supLessons := Result.A['Items'];
  CountOfLessonsInTT := 0;
  for I := 0 to LessCount - 1 do begin
    for J := 0 to Lessons[I].Count - 1 do begin
      ParentIndex := Lessons[I][J].Parent.ItemIndex;
      supLesson:=Subjects.ItemAsJsonObject[ParentIndex];
      supLesson.I['Parent'] := ParentIndex;
      supLesson.I['LessonIndex'] := Lessons[I][J].LessonIndex;
      supLessons.Add(supLesson);
      inc(CountOfLessonsInTT);
    end;
  end;
  Result.I['Count'] := CountOfLessonsInTT;
end;
...
function TTimeTable.GetForKlasses(KlassIndex, LessonIndex: Integer): TLesson;
var
//  Klass: TKlass;
  I: Integer;
begin
...
procedure TTimeTable.Add(LessonIndex: Integer; Lesson: TLesson);
begin
 // Выйти если нечего добавлять
  if Lesson = nil then
    Exit;
  // Выйти если не определён класс добавляемого предмета
  if Lesson.Klass = nil then
    Exit;
  // Выйти если занятие ни кто не ведёт
  if Lesson.TeacherCount=0 then Exit;
  AddToState(LessonIndex, Lesson);
...
procedure TTimeTable.SetAsJsonObject(const Value: ISuperObject);
begin
...
begin
 // Выйти если нечего добавлять
  ...
  Lesson := FLessons[LessonIndex].CreateLesson(Subject);
  AddToState(LessonIndex, Lesson);
end;
...
begin
  ...
  for I := 0 to Lessons[LessonIndex].Count - 1 do
    if Lessons[LessonIndex][I].Parent = Subject then begin
...
procedure TTimeTable.AddToState(LessonIndex: Integer; var Lesson: TLesson);
var
  I: Integer;
begin
  // Увеличить сумму часов предмета в расписании
  Lesson.InTimeTable := Lesson.InTimeTable + 1;
  // Отметить номер урока как занятый
  Lesson.Klass.Cross := Lesson.Klass.Cross + [LessonIndex];
  // Проделать то же самое данных Учителей и Кабинетов
  for I := 0 to Lesson.TeacherCount - 1 do
  begin
    Lesson.Teachers[I].IncCross(LessonIndex);
    Lesson.Kabinets[I].IncCross(LessonIndex);
  end;
end;

procedure TTimeTable.DeleteFromState(LessonIndex: Integer; Lesson: TLesson);
var
  I: Integer;
begin
  Lesson.Klass.Cross := Lesson.Klass.Cross - [LessonIndex];
  ...
  for I := 0 to Lesson.TeacherCount - 1 do
  begin
    Lesson.Teachers[I].DecCross(LessonIndex);
    Lesson.Kabinets[I].DecCross(LessonIndex);
  end;
end;
...
begin
  ...
  if Subject.Klass = nil then
      Exit;
  DeleteFromState(LessonIndex, Lesson);
...
begin
  ...
  for I := 0 to supSubjects.Length - 1 do begin
    Subject := Add;
    ItemAsJsonObject[Subject.ItemIndex] := supSubjects[I];
...
procedure TSubjects.SetItemAsJsonObject(Index: Integer;
...
begin
  ...
  Subject:=Item[Index];
  Subject.Klass := Klasses[Value.I['KlassIndex']];
  supTeachKabs := Value.A['Items'];
  for I := 0 to supTeachKabs.Length - 1 do begin
    supTeachKab := supTeachKabs[I];
    Teacher := Teachers[supTeachKab.I['TeacherIndex']];
    Kabinet := Kabinets[supTeachKab.I['KabinetIndex']];
    Subject.Add(Teacher, Kabinet);
  end;
  Subject.FComplexion := Value.I['Complexion'];
  Subject.FLessonAtWeek := Value.I['LessonAtWeek'];
  Subject.FNameIndex := Value.I['NameIndex'];
  Subject.FMultiLine := Value.B['MultiLine'];
end;
...
function TSubjects.GetItemAsJsonObject(Index: Integer): ISuperObject;
var
  I: Integer;
  N: Integer;
  supTeachKabs: TSuperArray;
...
begin
  ...
  with Item[Index] do begin
    ...
    if Klass = nil then
      ...
      N := Klass.ItemIndex;
    Result.I['KlassIndex'] := N;
    Result.I['TeacherCount'] := TeacherCount;
    Result.O['Items'] := SO('[]');
    supTeachKabs := Result.A['Items'];
    for I := 0 to TeacherCount - 1 do begin
      supTeachKab := SO;
      if Teachers[I] = nil then
        N := -1
      else
        N := Teachers[I].ItemIndex;
      supTeachKab.I['TeacherIndex'] := N;

      if Kabinets[I] = nil then
        N := -1
      else
        N := Kabinets[I].ItemIndex;
      supTeachKab.I['KabinetIndex'] := N;
      supTeachKabs.Add(supTeachKab);
    end;
    Result.I['Complexion'] := FComplexion;
    Result.I['LessonAtWeek'] := FLessonAtWeek;
    Result.I['NameIndex'] := FNameIndex;
    Result.B['MultiLine'] := FMultiLine;
//    Result.I['ItemIndex'] := ItemIndex;
  end;
end;
...
function TLesson.GetLongName: string;
begin
  Result := Parent.LongName;
end;

function TLesson.GetNameIndex: Integer;
begin
  Result := Parent.NameIndex;
end;

function TLesson.GetSanPIN: Integer;
begin
  Result := Parent.SanPIN;
end;

function TLesson.GetShortName: string;
begin
  Result := Parent.GetShortName;
end;
